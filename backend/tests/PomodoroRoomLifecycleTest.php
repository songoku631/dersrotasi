...
$pdo->exec("ALTER TABLE user_profiles ADD COLUMN username VARCHAR(24), ADD COLUMN first_name VARCHAR(80), ADD COLUMN last_name VARCHAR(80), ADD COLUMN profile_photo_path VARCHAR(255)");
...
$profileMessage=$repo->sendMessage('profile-owner',$profileRoom['id'],['message'=>'Profil mesajı']);
lifecycleCheck($profileMessage['username']==='test_student' && $profileMessage['profile_photo_url']==='/photos/test.png','Chat mesajı profil kullanıcı adı ve fotoğraf yolunu taşımalı.');
...
$pdo->prepare("INSERT INTO user_profiles(firebase_uid,first_name,last_name) VALUES('named-owner','Hayri','Kaya')")->execute();
$namedRoom=$repo->create('named-owner',[...$payload,'name'=>'İsim fallback odası']);
lifecycleCheck($namedRoom['members'][0]['username']==='Hayri Kaya' && $namedRoom['members'][0]['profile_photo_url']===null,'Username boşsa profil adı güvenli fallback olmalı.');
$namedMessage=$repo->sendMessage('named-owner',$namedRoom['id'],['message'=>'Merhaba']);
lifecycleCheck($namedMessage['username']==='Hayri Kaya' && $namedMessage['profile_photo_url']===null,'Chat mesajı profil adı fallback bilgisini taşımalı.');
