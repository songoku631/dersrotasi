...
import { defaultRoomName, formatTimer, remainingSeconds, validateRoom } from '../utils/pomodoro'

function freshForm(name = 'Öğrenci odası') {
  return { name, category: 'TYT', description: '', visibility: 'public', password_protected: false, password: '', password_confirmation: '', work_minutes: 25, break_minutes: 5, max_members: 12, voice_enabled: true, music_enabled: true }
}
...
function PomodoroPage() {
  ...
  useEffect(() => { const controller = new AbortController(); setLoading(true); listRooms(user, filter, controller.signal).then(response => { setRooms(response.data.items); setStats(response.data.stats); setError('') }).catch(requestError => requestError.name !== 'AbortError' && setError(requestError.message)).finally(() => setLoading(false)); return () => controller.abort() }, [user, filter])
  ...
  async function submit(event) {
    ...
    const payload = { ...form }; delete payload.password_confirmation; if (!payload.password_protected) delete payload.password
    try {
